...
//
// Oliver, on the island calendars: "having all their small 30
// participants-events on our event line seems silly ... we somehow need to keep
// these events hidden, but present." Asked what to call them: "consider it a
// community event. So along with the facebook ones."
//
// Major and Local both publish. Community does not: liveContent.js keeps those
// rows out of `events` and `majorEvents` entirely, so no page, no month chip,
// no front page line and no chat prompt can reach them, and a guide reads them
// only for a stop that is in that place.
//
// READ FROM THE FRONT OF THE WORD, like the two before it, so "Community
// event", "community" and "Community (island)" all land. Anything else is
// Local, which is what this line has always done with an answer it did not
// recognise: hiding a row has to be asked for.
export const festivalScale = (said) => {
...
const shapeForLiveFields = (type, t) => {
  ...
  if (type === "town") return { name: t.name, photo: `/towns/${slugify(t.name)}.jpg`, region: t.region || "", emoji: t.emoji || "📍", tag: t.tag || "", desc: t.characterAndFit, highlight: t.highlight || "", travelTime: t.travelTime || "", mapHint: t.mapHint || `${t.name}, Denmark`, nomiPotential: t.nomiPotential || "Medium", tier: t.tier || "", __lat: Number(t.lat) || null, __lon: Number(t.lon) || null,
    ...
    // ── AND WHICH ISLAND, WHEN THE ENTRY KNOWS ────────────────────
    // See islandOf in utils/geography.js. The kommune table places the islands
    // that are their own kommune and nothing else, so Sejerø, Møn, Strynø and
    // Anholt all come back as the mainland they are nearest to. A stated value
    // goes ahead of both derivations.
    //
    // CLEANED ON THE WAY IN, unlike the three above it, and that is a
    // difference on purpose rather than an inconsistency left lying: this is a
    // new field with nothing published under it, so it can start out holding
    // only place names. partOf and dayTripFrom carry rows that already say
    // "Day trip from Copenhagen", and cleaning those here would silently
    // rewrite live values on the next republish rather than in the editor where
  ...
  if (type === "festival") return { name: t.name, tier: t.tier || "", nearestStation: t.nearestStation || "", ticketInfo: t.ticketInfo || "", ticketPricedFor: String(t.ticketPricedFor || "").trim(), camping: t.camping || "", accommodationTip: t.accommodationTip || "", travelTime: t.travelTime || "", ticketStatus: normaliseTicketStatus(t.ticketStatus), town: t.town || "", type: t.type || "Festival", emoji: t.emoji || "🎪", date: t.dateStart || "", dateEnd: t.dateEnd || "", photo: `/events/${slugify(t.name)}.jpg`, desc: t.desc, mapHint: t.mapHint || "", website: t.website || "", color: t.color || "#8E24AA", tags: Array.isArray(t.tags) ? t.tags.slice(0, 3) : [], __scale: festivalScale(t.scale), towns: Array.isArray(t.towns) ? t.towns.filter(Boolean).slice(0, 2) : [], gemlyxFind: t.gemlyxFind || "",
    blogBody: [
  ...
  if (type === "essential") return { name: t.name, category: cleanCategory(t.category), emoji: t.emoji || "✨",
    ...
    // ── AND WHERE IT APPLIES, WHEN IT IS NOT THE WHOLE COUNTRY ────
    //
    ...
    //
    // Odense Letbane and FynBus Tourist are both on the national list because
    // there has never been a field that could say otherwise. See
    // utils/essentialPlace.js for why this is a SCOPE and not a town: FynBus
    // serves all of Funen and belongs on four town pages, not one.
    //
    // EMPTY IS NATIONAL, which is every published row today, so adding this
    // moves nothing until somebody types a place into the box.
    scope: cleanRelation(t.scope),
    blogBody: [
...
export const shapeForLive = (type, t) => {
  ...
  // ── AND HOW YOU GET TO THE ISLAND AT ALL ─────────────────────────
  //
  // Oliver, 20 Sep 2026: "Like its website." The operator's NAME has been on
  // every island since the type existed and the link has never been anywhere,
  // because nothing carried it: the drafting prompt reads the operator's own
  // page for the crossing time under a mandatory rule and then keeps only the
  // number.
  //
  // Declared here in the same commit that starts writing it, which is the rule
  // this function has now been taught seven times. Validated on the way in for
  // the reason ticketUrl is: this is the one link in the app where the wrong
  // page costs a day rather than a click, and a tourist board's copy of last
  // season's timetable is exactly what a search returns first. See
  // utils/ferryDoor.js.
  if (isOperatorSite(t?.ferryUrl)) out = { ...out, ferryUrl: String(t.ferryUrl).trim() };
