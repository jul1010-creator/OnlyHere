...
IF THIS VENUE IS INSIDE A HOSTEL, HOTEL, KRO OR CAMPSITE, YOU ARE WRITING ABOUT THE BAR. This is common in Denmark and the bar is the reason this entry exists: a hostel bar the town drinks in is nightlife, and the beds are not. Your search results will be overwhelmingly about the accommodation, because that is what the internet has to say about a hostel — follow them and you will file a booking review under Nightlife.
...
DO say, in one clause, that the bar is inside a hostel — that is a real fact and it changes what walking in feels like. And answer the ONE question that only this kind of venue raises, which is the single most useful thing you can tell a reader here: CAN SOMEBODY WHO IS NOT STAYING THERE WALK IN AND DRINK? Say so plainly either way if the research supports it; if it does not say, put that in uncertainties rather than guessing, because a reader who is turned away at the door was failed by this entry.
