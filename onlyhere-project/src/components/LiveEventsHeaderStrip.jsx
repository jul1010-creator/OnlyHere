  ...
  return (
    <div style={{ marginTop: 4, marginBottom: 2 }}>
      {(currentlyLive.length > 0 || comingSoon.length > 0) && (
        <>
          {/* Modernized: the old "LIVE | COMING" text links with a literal pipe
              character are now one proper segmented control — active segment gets
              the light fill + dark text, matching the app's Pill language. */}
          <div style={{ display: "inline-flex", alignItems: "center", background: "rgba(33,44,68,0.45)", border: `1px solid ${C.border}`, borderRadius: 100, padding: 3, marginBottom: 10 }}>
            ...
            {currentlyLive.length > 0 && (
              <button onClick={() => setSegment("live")} disabled={!showBoth}
                style={{ display: "flex", alignItems: "center", gap: 6, background: isLive ? "#EDF0F7" : "none", border: "none", borderRadius: 100, padding: "5px 12px", cursor: showBoth ? "pointer" : "default", fontFamily: "'Inter', sans-serif" }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#4CAF50", flexShrink: 0, boxShadow: "0 0 6px #4CAF50", animation: "gemlyxLiveDotPulse 1.6s ease-in-out infinite" }} />
                <span style={{ fontSize: 10.5, fontWeight: 700, color: isLive ? "#0A0F1E" : "#4CAF50", textTransform: "uppercase", letterSpacing: 0.5 }}>Live Events</span>
              </button>
            )}
            {comingSoon.length > 0 && (
              <button onClick={() => setSegment("coming")} disabled={!showBoth}
                style={{ display: "flex", alignItems: "center", gap: 6, background: !isLive ? "#EDF0F7" : "none", border: "none", borderRadius: 100, padding: "5px 12px", cursor: showBoth ? "pointer" : "default", fontFamily: "'Inter', sans-serif" }}>
                <span style={{ fontSize: 10.5, fontWeight: 700, color: !isLive ? "#0A0F1E" : C.gold, textTransform: "uppercase", letterSpacing: 0.5 }}>{comingLabel}</span>
              </button>
            )}
          </div>
      ...
      {!nearYou && (
        <button onClick={requestLocation} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", padding: "4px 0", cursor: "pointer", fontFamily: "'Inter', sans-serif" }}>
          <Ico name="pin" size={13} color={C.light} />
